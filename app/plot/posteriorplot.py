...
def posteriorplot(x, y1, y2, lower_bound, upper_bound):
    ...
    testBeatsControl = np.where(y2 == y2.max())[0][0] > np.where(y1 == y1.max())[0][0]

    if testBeatsControl: # test > control
